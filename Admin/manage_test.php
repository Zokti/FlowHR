...
<body>
    ...
    <div class="container">
        ...
        <!-- Секция для тестов -->
        <h2><i class="fas fa-file-alt"></i> Тесты</h2>
        <button class="btn-action" data-bs-toggle="modal" data-bs-target="#addTestModal">
            ...
            <tbody>
                <?php foreach ($tests as $test): ?>
                    <tr>
                        ...
                        <td>
                            <button class="btn-action" data-bs-toggle="modal" data-bs-target="#editTestModal" data-test-id="<?= $test['id'] ?>" data-test-title="<?= htmlspecialchars($test['title']) ?>" data-test-time-limit="<?= htmlspecialchars($test['time_limit']) ?>">
                                <i class="fas fa-edit"></i> Редактировать
                            </button>
                            <button class="btn-action btn-delete" data-bs-toggle="modal" data-bs-target="#deleteTestModal" data-test-id="<?= $test['id'] ?>">
        ...
        <button class="btn-action" data-bs-toggle="modal" data-bs-target="#addQuestionModal">
            ...
            <tbody>
                <?php foreach ($questions as $question): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($question['test_id']) ?></td>
                        <td><?= htmlspecialchars($question['max_score']) ?></td>
                        <td>
                            <button class="btn-action" data-bs-toggle="modal" data-bs-target="#editQuestionModal" data-question-id="<?= $question['id'] ?>" data-question-text="<?= htmlspecialchars($question['question_text']) ?>" data-test-id="<?= htmlspecialchars($question['test_id']) ?>">
                                <i class="fas fa-edit"></i> Редактировать
                            </button>
                            <button class="btn-action btn-delete" data-bs-toggle="modal" data-bs-target="#deleteQuestionModal" data-question-id="<?= $question['id'] ?>">
                                <i class="fas fa-trash"></i> Удалить
                            </button>
                        </td>
                    </tr>
        ...
        <!-- Секция для ответов -->
        <h2><i class="fas fa-check-circle"></i> Ответы</h2>
        <button class="btn-action" data-bs-toggle="modal" data-bs-target="#addAnswerModal">
            ...
            <tbody>
                <?php foreach ($answers as $answer): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($answer['question_id']) ?></td>
                        <td><?= $answer['is_correct'] ? 'Да' : 'Нет' ?></td>
                        <td>
                            <button class="btn-action" data-bs-toggle="modal" data-bs-target="#editAnswerModal" data-answer-id="<?= $answer['id'] ?>" data-answer-text="<?= htmlspecialchars($answer['answer_text']) ?>" data-question-id="<?= htmlspecialchars($answer['question_id']) ?>" data-is-correct="<?= htmlspecialchars($answer['is_correct']) ?>">
                                <i class="fas fa-edit"></i> Редактировать
                            </button>
                            <button class="btn-action btn-delete" data-bs-toggle="modal" data-bs-target="#deleteAnswerModal" data-answer-id="<?= $answer['id'] ?>">
                                <i class="fas fa-trash"></i> Удалить
                            </button>
                        </td>
                    </tr>
    ...
    <?php include 'modals/tests_modals.php'; ?>

    <!-- Модальные окна для вопросов -->
    <?php include 'modals/questions_modals.php'; ?>

    <!-- Модальные окна для ответов -->
    <?php include 'modals/answers_modals.php'; ?>
...
document.getElementById('editQuestionForm').addEventListener('submit', function (event) {
    ...
    fetch('admin_actions.php?action=edit_question', {
        method: 'POST',
        body: formData
    }).then(response => {
        if (response.ok) {
